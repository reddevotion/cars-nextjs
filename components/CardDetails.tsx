    ...
    <>
        <Transition appear show={isOpen} as={Fragment}>
            <Dialog as="div" className="relative z-10" onClose={closeModal}>
                <TransitionChild as={Fragment} enter='ease-out duration-300' enterFrom='opacity-0' enterTo='opacity-100' leave='ease-in duration-300' leaveFrom='opacity-100' leaveTo='opacity-0'>
                            ...
                            <DialogPanel className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white p-6 text-left shadow-xl transition-all flex flex-col gap-5">
                                ...
                                <div className='flex-1 flex flex-col gap-3'>
                                    <div className='relative w-full h-40 bg-pattern bg-cover bg-center rounded-lg flex items-center justify-center'>
                                        <Image src={generateImageUrl(car)} width={400} height={200} alt="car" className='object-contain'/>
                                    </div>
                                    <div className='flex gap-3'>
                                        <div className='flex-1 w-full relative h-24 bg-primary-blue-100 rounded-lg'>
                                        <Image src={generateImageUrl(car, '29')} fill alt="car" className='object-contain'/>
                                        </div>
                                        <div className='flex-1 w-full relative h-24 bg-primary-blue-100 rounded-lg'>
                                        <Image src={generateImageUrl(car, '33')} fill alt="car" className='object-contain'/>
                                        </div>
                                        <div className='flex-1 w-full relative h-24 bg-primary-blue-100 rounded-lg'>
                                        <Image src={generateImageUrl(car, '13')} fill alt="car" className='object-contain'/>
                                        </div>
                                    </div>
                                </div>
